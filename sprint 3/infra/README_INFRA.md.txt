Infraestructura local – Sprint 3 (Digital Money House)

Este repositorio proporciona la infraestructura necesaria para arrancar el backend del proyecto Digital Money House en el Sprint 3, junto con una base de datos MySQL utilizando Docker Compose. El objetivo es que cualquier miembro del equipo —especialmente QA— pueda levantar el stack completo de forma reproducible y sin ajustes manuales y probar los nuevos endpoints de la actividad, dashboard y transferencias.
...
Pruebas de QA para Sprint 3

En este sprint se amplía la funcionalidad con dashboard, filtros de actividad y la posibilidad de registrar ingresos. Además se mantiene la gestión de tarjetas y la consulta de saldo y movimientos. La siguiente guía describe cómo probar estos endpoints usando Swagger una vez que el stack esté en ejecución:
...
Además del entorno local, Sprint 3 contempla el despliegue del frontend en Vercel y la posibilidad de almacenar imágenes (por ejemplo, fotos de tarjetas o comprobantes) en Amazon S3.
